...
#define ddt32c2s_i2c_device_num(c){                                         \
        .name                   = "delta-ddt32c2s-i2c-device",                \
        .id                     = c,                                        \
        .dev                    = {                                           \
                    .platform_data = &ddt32c2s_i2c_device_platform_data[c], \
...
static struct i2c_device_platform_data ddt32c2s_i2c_device_platform_data[] = {
    {
        /* psu 1 (0x58) */
        .parent = 1,
        .info = { .type = "dni_ddt32c2s_psu", .addr = 0x58, .platform_data = (void *) 0 },
        .client = NULL,
    },
    {
        /* psu 2 (0x58) */
        .parent = 2,
        .info = { .type = "dni_ddt32c2s_psu", .addr = 0x58, .platform_data = (void *) 1 },
        .client = NULL,
    },
    {
...
static struct platform_device ddt32c2s_i2c_device[] = {
    ddt32c2s_i2c_device_num(0),
    ddt32c2s_i2c_device_num(1),
    ddt32c2s_i2c_device_num(2),
    ddt32c2s_i2c_device_num(3),
    ddt32c2s_i2c_device_num(4),
    ddt32c2s_i2c_device_num(5),
    ddt32c2s_i2c_device_num(6),
    ddt32c2s_i2c_device_num(7),
    ddt32c2s_i2c_device_num(8),
    ddt32c2s_i2c_device_num(9),
    ddt32c2s_i2c_device_num(10),
};
...
static int __init delta_ddt32c2s_platform_init(void)
{
    ...
    printk("ddt32c2s_platform module initialization\n");
...
//    i2c_client_9548 = i2c_new_device(adapter, &i2c_info_pca9548[0]);
    ...
    for (cnt = 0; cnt < ARRAY_SIZE(ddt32c2s_i2c_device); cnt++)
    {
        ret = platform_device_register(&ddt32c2s_i2c_device[cnt]);
        if (ret) {
            printk(KERN_WARNING "Fail to create i2c device %d\n", cnt);
            goto error_ddt32c2s_i2c_device;
        }
    }

    return 0;

error_ddt32c2s_i2c_device:
    for (; cnt >= 0; cnt--) {
        platform_device_unregister(&ddt32c2s_i2c_device[cnt]);
    }
...
static void __exit delta_ddt32c2s_platform_exit(void)
{
    int i = 0;

    for ( i = 0; i < ARRAY_SIZE(ddt32c2s_i2c_device); i++ ) {
        platform_device_unregister(&ddt32c2s_i2c_device[i]);
    }
...
module_init(delta_ddt32c2s_platform_init);
module_exit(delta_ddt32c2s_platform_exit);

MODULE_DESCRIPTION("DNI ddt32c2s Platform Support");
