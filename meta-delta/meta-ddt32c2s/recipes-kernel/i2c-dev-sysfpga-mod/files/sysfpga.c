...
static ssize_t i2c_fpga_read(struct device *dev, const int reg, uint32_t *reg_val)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct sysfpga_data_s *sysfpga_data = i2c_get_clientdata(client);
    i2c_dev_data_st *data = sysfpga_data->i2c_dev_data;
...
static ssize_t i2c_fpga_write(struct device *dev, const int reg, uint32_t result)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct sysfpga_data_s *sysfpga_data = i2c_get_clientdata(client);
    i2c_dev_data_st *data = sysfpga_data->i2c_dev_data;
...
static int sysfpga_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
    ...
    struct sysfpga_data_s *sysfpga_data;
    //i2c_dev_data_st *sysfpga_attr_data;
    int ret = 0;

    sysfpga_data = devm_kzalloc(dev, sizeof(struct sysfpga_data_s), GFP_KERNEL);
    if (!sysfpga_data) {
        return -ENOMEM;
    }
    SYSFPGA_DEBUG("Allocate sysfpga_data");
    sysfpga_data->i2c_dev_data = devm_kzalloc(dev, sizeof(i2c_dev_data_st), GFP_KERNEL);
    if (!sysfpga_data->i2c_dev_data) {
        return -ENOMEM;
    }
    SYSFPGA_DEBUG("client->name %s", &client->name);

    ret = i2c_dev_sysfs_data_init(client, sysfpga_data->i2c_dev_data, sysfpga_attr_table, n_attrs);
    if (ret) {
        return ret;
    }
...
{
    struct sysfpga_data_s *sysfpga_data = i2c_get_clientdata(client);
    i2c_del_adapter(&sysfpga_data->adap);
    SYSFPGA_DEBUG("Delete i2c adapter");
    i2c_dev_sysfs_data_clean(client, sysfpga_data->i2c_dev_data);
    return 0;
}
